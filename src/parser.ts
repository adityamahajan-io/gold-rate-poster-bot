...
export function parseRateMessage(text: string): ParsedRates {
  ...
  // Extract date - pattern: "As on March 30th, 2026 - Time"
  const dateMatch = normalized.match(/As on\s+(.+?)\s+-\s+Time/i);
  if (!dateMatch) {
    throw new ParseError('Could not find date. Expected format: "As on March 30th, 2026 - Time"');
  }
  const date = dateMatch[1].trim();

  // Extract time - pattern: "Time 18.37 Hrs"
  const timeMatch = normalized.match(/Time\s+(\d{1,2}\.\d{2})\s+Hrs/i);
  if (!timeMatch) {
    throw new ParseError('Could not find time. Expected format: "Time 18.37 Hrs"');
  }
  const time = timeMatch[1];

  // Find Gold Sale Rate section (between "Gold Sale Rate" and "Gold Purchase Rate")
  const goldSaleMatch = normalized.match(/Gold Sale Rate\s+([\s\S]+?)\s+Gold Purchase Rate/i);
  if (!goldSaleMatch) {
    throw new ParseError('Could not find Gold Sale Rate section');
  }
  const goldSection = goldSaleMatch[1];

  // Extract gold rates from the section
  // Format: "1,49,200 : Standard (99.5)"
  ...
  // Find Silver Sale Rate section (between "Silver Sale Rate" and "Silver Purchase Rate")
  const silverSaleMatch = normalized.match(/Silver Sale Rate\s+([\s\S]+?)\s+(?:Silver Purchase Rate|Platinum)/i);
  if (!silverSaleMatch) {
    throw new ParseError('Could not find Silver Sale Rate section');
  }
  const silverSection = silverSaleMatch[1];

  // Extract silver bullion rate
  // Format: "2,34,500 : for Bullion and Pure Utensils and Coin/Bar"
